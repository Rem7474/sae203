    ...
    $('#commander').on('submit', function(e) {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          $('.panier').hide();
          $('#resultats').html(response);
          window.location.href = 'succes.php?success=Commande Numéro '+response+' effectuée avec succès&page=mescommandes';
        }
      });
    });
    //Supression de l'article dans le panier
    $('.supprimer').on('submit', function(e) {
      e.preventDefault(); // Empêche le formulaire de se soumettre normalement
  
      // Récupération des données du formulaire
      var articleId = $(this).find('.idarticle').val();
      // Appel AJAX
      $.ajax({
        type: 'GET',
        url: 'modifpanier.php',
